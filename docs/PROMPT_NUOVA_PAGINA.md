...
1. **GUARDA page.png** - `assets/images/{libro}/p0XX/page.png` - FONDAMENTALE per capire layout e struttura visiva!
2. **Leggi HTML originale** - `assets/images/{libro}/p0XX/*.html` - per estrarre testo e risposte
...
7. **Crea la view** - Interattiva completa
...
**Testo + immagine (flex per colonne non uguali):**
```erb
...
**Flex con proporzioni (meglio di grid per colonne diverse):**
```erb
<div class="flex flex-col md:flex-row gap-6">
  <div class="md:w-3/5"><!-- Contenuto principale --></div>
  <div class="md:w-2/5"><!-- Contenuto secondario --></div>
</div>
```
...
<div class="text-right">
  <span class="text-pink-600 font-bold">Quaderno esercizi</span> p. 140
</div>
```
...
```erb
<div class="text-right">
  <span class="text-pink-600 font-bold">Quaderno esercizi</span> pp. 141, 142
</div>
```

### Dove trovare il riferimento:
Cercare nell'HTML originale in `app/assets/images/{libro}/pXXX/*.html` la stringa "Quaderno"
...
### Metodo consigliato (dati dal database):
...
- `sottotitolo`: categoria (es. "FRAZIONI", "NUMERI")
- `base_color`: colore tema (es. "blue", "purple", "pink")
...
## TIPOLOGIE DI ESERCIZI RICONOSCIUTE
...
- [ ] Container `bg-white` + data-controllers (`exercise-checker text-toggle font-controls`)
...
- [ ] Colori dinamici con `@pagina.base_color` o classi custom Tailwind 4
- [ ] Input con stile appropriato (bordo o dotted)
...
- [ ] Box colorati corretti (`bg-pink-light`, `bg-custom-blue`, ecc.)
- [ ] Controller Stimulus appropriati
- [ ] Nessun tag `<u>` - usare `<span class="text-cyan-600 font-bold">`
- [ ] Link Quaderno allineato a destra (`text-right`) se presente
