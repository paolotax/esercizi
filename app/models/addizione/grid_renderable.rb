...
# Metodi per il rendering della griglia quaderno per addizioni
# Genera la matrice per il partial unificato _quaderno_grid
module Addizione::GridRenderable
  ...
  def show_addend?(index)
    if show_addend_indices.present?
  ...
  def to_s
    parsed_addends.join(" #{current_operator} ")
  end

  # Stringa dell'operazione (es. "123 + 456 =")
  ...
  def to_grid_matrix
    ...
    display_title = show_addends? ? title : (title.presence || operation_string)
  ...
  def show_addends?
    show_addends.nil? ? false : show_addends
  end
  ...
  def build_addend_row(addend_idx, column_types, total_cols, cell_size)
    ...
    column_types.each do |col_type|
      case col_type
      when :sign
        sign_value = addend_idx == parsed_addends.length - 1 ? "=" : current_operator
        cells << {
          type: :sign,
          value: sign_value,
          classes: "text-blue-600 dark:text-blue-400"
