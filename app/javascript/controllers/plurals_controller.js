...
// Connects to data-controller="plurals"
export default class extends Controller {
  static targets = ["input"]

  connect() {
    console.log("Plurals controller connected")
  }
  ...
  showFeedback(allCorrect, correctCount, incorrectCount, emptyCount) {
    const existingFeedback = this.element.querySelector(".plurals-feedback")
    if (existingFeedback) {
      existingFeedback.remove()
    }

    const feedbackDiv = document.createElement("div")
    feedbackDiv.className = "plurals-feedback mt-6 p-6 rounded-lg shadow-lg " +
  ...
  reset() {
    ...
    const feedback = this.element.querySelector(".plurals-feedback")
